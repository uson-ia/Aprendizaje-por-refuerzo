...
Los problemas de Inventario son un tipo de problema ejemplo muy comun en control
estocástico. En este modulo se considera en este caso un inventario con las
...
- Los pedidos (acciones) tienen un máximo ``A_max`` (default ``A_max = maxint``)

- Las acciones admisibles en el estado ``s`` son A(s) = {0, ... , min(M + B - s,
  A_max)}
...
- La demanda es una variable aleatoria que se genera a partir de una
  distribución ``demanda(x)`` Si no se especifica ``demanda(x)``, por default se
  utiliza una distribución uniforme entre 0 y ``M + B``. Para este problema se
  considera la demanda estacionaria, por lo que su distribución no cambia en el
  tiempo.
...
class Inventario(mdp.MDP):
    """
    Clase para el problema de inventario descrito en la explicación general del módulo
