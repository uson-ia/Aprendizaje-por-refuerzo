#libreria Tkinter es para la interfaz grafica
from Tkinter import *
master = Tk()
...
#walls: son las paredes negras de en medio que se aprecian en el cuadro
walls = [(1, 1), (1, 2), (2, 1), (2, 2)]
#Specials son los cuadros rojos y verdes y sus coordenadas el verde es el estado meta y el rojo es
...
def create_triangle(i, j, action):
    #Si la accion es UP: se pinta el triangulo de arriba de cada cuadrante
    if action == actions[0]:
        return board.create_polygon((i+0.5-triangle_size)*Width, (j+triangle_size)*Width,
                                    ...
                                    (i+0.5)*Width, j*Width,
                                    fill="white", width=1)
    #Si la accion es DOWN: se pinta el triangulo de abajo de cada cuadrante
        ...
        return board.create_polygon((i+0.5-triangle_size)*Width, (j+1-triangle_size)*Width,
                                    ...
                                    fill="white", width=1)
     #Si la accion es LEFT: se pinta el triangulo de la izquierda de cada cuadrante
        ...
        return board.create_polygon((i+triangle_size)*Width, (j+0.5-triangle_size)*Width,
                                    ...
                                    i*Width, (j+0.5)*Width,
                                    fill="white", width=1)
    #Si la accion es RIGHT: se pinta el triangulo de la derecha de cada cuadrante
...
def render_grid():
    ...
    #tamano de la cuadricula es decir (x=5)*(y=5)
    ...
    for i in range(x):
        for j in range(y):
            ...
            temp = {}
            #aqui se crean los triangulos pequenos que sirven solo para diseno
            for action in actions:
                temp[action] = create_triangle(i, j, action)
            cell_scores[(i,j)] = temp
        #se pinta el cuadro verde y el rojo
...
#estableciendo el puntaje por celda
